...
const { BOOKING_SOP } = require('../ai/prompts/booking.prompt');
...
class BookingService {
  async evaluate(conversation) {
    try {
      logger.info('Compiling Booking & Ticketing Prompt...');

      // 1. Combine the Global Guardrails with the specific Booking Rules
      ...
      if (typeof rawText === 'string') {
        ...
        if (llmResponse?.tokenUsage) {
          logger.info("LLM Usage Tracked (Booking)", {
      ...
      logger.error('BookingService Evaluation Failed:', error.message);
